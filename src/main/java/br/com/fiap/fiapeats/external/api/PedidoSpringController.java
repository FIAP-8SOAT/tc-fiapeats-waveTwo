...
public class PedidoSpringController {
    ...
    @Operation(
            ...
            description = "Rota para listar todos os pedidos cadastrados filtrados por pagamento")
    @ApiResponses(
            value = {@ApiResponse(responseCode = "200", description = "Pedidos listados com sucesso!")})
    public ResponseEntity<List<ListarPedidosResponse>> listarPedidosPorPagamento(@PathVariable Long idPagamento) {
        ThreadContext.put(Constants.CORRELATION_ID, UUID.randomUUID().toString());
        log.info(
                "correlationId={"
                        + ThreadContext.get(Constants.CORRELATION_ID)
                        + "} "
                        + "Solicitacao recebida [ListarPedidosPorPagamento] ");
        return ResponseEntity.ok(pedidoController.listarPedidosPorPagamento(idPagamento));
    }
    ...
    public ResponseEntity<ListarPedidosResponse> listarPedidoPorId(@PathVariable String idPedido) {
        ThreadContext.put(Constants.CORRELATION_ID, UUID.randomUUID().toString());
        log.info(
                "correlationId={"
                        + ThreadContext.get(Constants.CORRELATION_ID)
